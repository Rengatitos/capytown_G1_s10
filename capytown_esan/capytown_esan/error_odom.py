...
Lee un ros2 bag con /odom y grafica la trayectoria estimada vs el cuadrado ideal.
...
SALIDA
------
    <bag>_trayectoria.png   — trayectoria /odom vs cuadrado ideal
...
def read_odom_from_bag(bag_path):
    """
    Lee todos los mensajes /odom de un ros2 bag (formato sqlite3).
...
def simulate_odom(n_laps=3, dt=0.02,
                  ...
                  seed=42):
    """
    Simula datos de /odom para n_laps vueltas al cuadrado de 1x1 m.
...
def plot_trajectory(xs, ys, run_name, out_png):
    """
    Grafica la trayectoria /odom vs el cuadrado ideal 1x1 m.
...
def main():
    ...
    if sys.argv[1] == "--sim":
        ...
        if not xs:
            print("ERROR: no se encontraron mensajes /odom en el bag.")
            print("  Verifica con: ros2 bag info " + bag_path)
            sys.exit(1)

        print(f"Mensajes /odom leidos: {len(xs)}")
