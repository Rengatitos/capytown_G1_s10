...
---
    ...
    python3 calibrar_robot.py

    # Con parámetros:
    python3 calibrar_robot.py --ros-args -p b_eff:=0.20

CÓMO FUNCIONA
-------------
1.  El script publica en /cmd_vel un giro en sitio (angular.z = ω).
2.  Lee /odom_raw continuamente, extrae el yaw del quaternion y acumula el ángulo.
...
REQUISITOS
----------
- ROS2 Humble instalado y entorno cargado
- El driver de bringup del Yahboom debe estar publicando /odom_raw y aceptando /cmd_vel
- ROS_DOMAIN_ID=20 configurado
...
copiar al YAML del bringup del robot (config/wheel_params.yaml).
    ...
    def _control_loop(self):
        ...
        if (now - self._t_start) > self.max_run_seconds:
            self.get_logger().warn(
                "Timeout de la corrida — deteniendo robot. Revisa /odom_raw y velocidad."
            )
    ...
    def _finalize_run(self, timed_out=False):
        ...
        if timed_out:
            L.warn(
                ...
                f"Reportado por /odom_raw: {reported_deg:.2f}°"
            )
        else:
            L.info("")
            L.info(f"Robot detenido. Reportado por /odom_raw: {reported_deg:.2f}°")
        ...
        L.info("-" * 64)
        L.info(f"  Reportado por /odom_raw : {reported_deg:8.2f}°")
